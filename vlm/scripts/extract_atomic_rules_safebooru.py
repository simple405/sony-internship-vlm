"""Extract atomic_rules.json from Safebooru images with Qwen VL.
...
DEFAULT_PROMPT = Path("vlm/prompts/supervision/atomic_rules_from_safebooru.txt")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Extract atomic rules from Safebooru images.")
...
def resolve_image_path(row: dict[str, str], output_root: Path) -> Path:
    """Resolve current and legacy manifest paths without rewriting the manifest."""
    ...
    fallback = root / "image" / Path(row["image_path"]).name
    if fallback.exists():
        return fallback
    return candidate
...
def main() -> None:
    ...
    with ThreadPoolExecutor(max_workers=max(1, args.workers if not args.dry_run else 1)) as pool:
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                (sample_dir / "error.json").write_text(json.dumps(error, ensure_ascii=False, indent=2), encoding="utf-8")
                result = error
    ...
    (args.output_root / "extraction_summary.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")
